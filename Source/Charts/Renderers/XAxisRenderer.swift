...
open class XAxisRenderer: AxisRendererBase
{
    ...
    @objc open func drawLabels(context: CGContext, pos: CGFloat, anchor: CGPoint)
    {
        ...
        let lastLabelsAttrs: [NSAttributedStringKey : Any] = [NSAttributedStringKey.font: xAxis.specialLabelsFont,
                                                              NSAttributedStringKey.foregroundColor: xAxis.specialLabelsTextColor,
                                                              NSAttributedStringKey.paragraphStyle: paraStyle]
        ...
        for i in stride(from: 0, to: entries.count, by: 1)
        {
            ...
            if viewPortHandler.isInBoundsX(position.x)
            {
                ...
                let y: CGFloat
                if isSpecialLabel {
                    let labelSizeWithCommonFont = label.size(withAttributes: [NSAttributedStringKey.font: xAxis.labelFont])
                    let labelSizeWithSpecialFont = label.size(withAttributes: [NSAttributedStringKey.font: xAxis.specialLabelsFont])
